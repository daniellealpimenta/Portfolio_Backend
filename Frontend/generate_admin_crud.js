  ...
  <div class="space-y-6 max-w-5xl">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-body text-mist text-sm">Gerencie ${entity.title.toLowerCase()}.</p>
      </div>
      <NuxtLink to="/admin/${entity.name}/new" class="btn-cta bg-periwinkle text-navy px-4 py-2 rounded-xl text-sm flex items-center gap-2 hover:opacity-90 transition-opacity">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
        Novo(a)
      </NuxtLink>
    </div>

    <div class="bg-navypanel border ink-border rounded-2xl overflow-hidden">
      <div v-if="loading" class="p-8 text-center text-meta text-mist animate-pulse">Carregando...</div>
      
      <table v-else-if="${entity.composableKey}.length > 0" class="w-full text-left border-collapse">
        <thead>
          <tr class="border-b ink-border bg-navy/50">
            <th class="py-4 px-6 text-meta text-xs text-mist font-medium">Item</th>
            <th class="py-4 px-6 text-meta text-xs text-mist font-medium text-right">Ações</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="item in ${entity.composableKey}" :key="item.id" class="border-b ink-border hover:bg-navy/30 transition-colors">
      ...
      <div v-else class="p-8 text-center text-body text-mist">
...
function generateNewVue(entity) {
  ...
  <div class="max-w-2xl">
    <div class="mb-6 flex items-center gap-4">
      ...
      <h1 class="text-h2 m-0 text-paper">Novo(a) ${entity.title}</h1>
    </div>
