...
def create_link(link_in: ProjectLinkCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    service = ProjectLinkService(db)
    return service.create_link(link_in, current_user.id)
...
def update_link(link_id: UUID, link_in: ProjectLinkUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    service = ProjectLinkService(db)
    return service.update_link(link_id, link_in, current_user.id)

@router.delete("/{link_id}")
def delete_link(link_id: UUID, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    service = ProjectLinkService(db)
    return service.delete_link(link_id, current_user.id)
